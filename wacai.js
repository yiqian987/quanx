...
项目名称：挖财记账 v2（新增开屏/插屏广告去除，规则与引用均不变）
使用声明：⚠️仅供参考，🈲转载与售卖！
...
远程引用还是原来那个 wacai.js，规则段**一字未动**（仍是整域名兜底）。
v2 只改脚本段：在脚本内部按 URL 分支，新增广告位处理。
...
下面是 v2 之外**没有做**的几件事，都记在这里免得以后重复问：
...
规则段与 v1 完全一致（整域名兜底），因此**不需要换引用、不需要删旧引用**。
